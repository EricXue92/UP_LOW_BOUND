...
class UpperLower_Control(keras.Model):
    ...
    def init_arguments(self, method = 'No_PCGrad', coverage_rate = 0.9, lamda = 10):
    ...
    def coverage_penalty(self, y_true, y_pred):
        coverage_loss = self.lamda * K.square( K.maximum(0.0, self.coverage_rate - K.mean(y_pred[:, -1]) ))
...
#   year={2020}}
    ...
    def train_step(self, data):
        ...
        if self.method == 'No_PCGrad':
            loss = 1/5*K.mean(width_up_loss)+1/5*K.mean(width_low_loss)+1/5*K.mean(up_penalty_loss)+1/5*K.mean(low_penalty_loss)+1/5*K.mean(coverage_penalty)
